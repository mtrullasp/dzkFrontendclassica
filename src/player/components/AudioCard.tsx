...
const AudioCard = (props: AudioCardProps) => {
  ...
  const { state, pause, setCurrentTime, load, play, stop, setProgress } =
    useMedia(getInitialState(props));
    ...
    <Card className={classes.card}>
      <CardContent className={classes.container}>
        <Grid container justifyContent="center" alignItems="center">
          <audio ref={player} key={state.key} className={classes.audio}>
            {state.url && (
              <source type={getMimeType(state.url)} src={state.url} />
            )}
          </audio>
          <Grid item xs={12}>
            <Progress
              ...
              color={color}
              thickness={thickness}
              onProgressClick={async (v) =>
                await setProgress(player.current, v)
              }
            />
          </Grid>
          <Grid item md={6} xs={12} className={classes.controls}>
            <MediaTime time={state.time} TimeProps={props.TimeProps} />
            <ControlKeys
          ...
          <Grid item md={6} sm={12} className={classes.bars}>
            {props.speed && (
              <SpeedBar
