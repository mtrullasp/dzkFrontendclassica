...
import ImageComposerOne from "../composer/ImageComposerOne";
import {
  Box,
  Button,
  Container,
...
import ImagePerformerOne from "./ImagePerformerOne";

export const WIDTH: number = 160;
const MARGIN_LEFT_CONTENT: number = 15;

export interface PerformerOneProps {
  title?: string;
  content: JSX.Element;
}
...
const PerformerOneTemplateIntern = (props: PerformerOneProps) => {
  ...
  // const [Performer, setPerformer] = useState<IPerformer>(props.Performer);
  ...
  return (
    ...
    >
      ...
      <ImagePerformerOne
        base64={store.performerAct?.imatgeBase64Large}
        imgStyle={imgStyle}
      />
      <Box
        style={{
        ...
        <Grid item lg={12} style={{ marginRight: 5 }}>
          <Stack>
            ...
            <MenuButton
              ...
              text={"Random"}
              onClick={() => {
                navigate("/RandomTrack/" + store.performerAct?.id);
              }}
            />
          </Stack>
        </Grid>
...
const PerformerOneTemplate = (props: PerformerOneProps) => {
  return (
    <LayoutContainer
      headerContent={
        <MainHeader showNextPrevious={true} showBack={true} showHome={true} />
      }
      clientContent={<PerformerOneTemplateIntern {...props} />}
...
export default PerformerOneTemplate;
